...
const scope = 'user-read-private user-read-email user-read-playback-state user-read-currently-playing user-modify-playback-state playlist-modify-public streaming';
...
app.get('/callback', async (req, res) => {
  ...
  if (state === null || !verifier) {
    ...
    try {
      ...
      console.error('Error during the authentication process:', error.response?.data || error.message);
...
app.get('/recommendations', async (req, res) => {
  ...
  if (!req.session.accessToken || !seed_tracks) {
    return res.status(403).json({ error: 'Access token or seed tracks are missing' });
  }

  try {
    const response = await fetch(`https://api.spotify.com/v1/recommendations?seed_tracks=${seed_tracks}&limit=1`, {
      headers: { 'Authorization': `Bearer ${req.session.accessToken}` },
    });
    ...
    if (response.status === 429) {
      ...
      return res.status(429).json({ error: 'Rate limit exceeded', retryAfter: retryAfter });
    }
...
app.post('/play', async (req, res) => {
  ...
  try {
    const response = await fetch(`https://api.spotify.com/v1/me/player/play?device_id=${encodeURIComponent(deviceId)}`, fetchOptions);
...
app.post('/pause', async (req, res) => {
  ...
  try {
    const response = await fetch(`https://api.spotify.com/v1/me/player/pause?device_id=${encodeURIComponent(deviceId)}`, {
      method: 'PUT',
      headers: {
        'Authorization': `Bearer ${accessToken}`,
        'Content-Type': 'application/json',
...
app.put('/volume', async (req, res) => {
  ...
  try {
      const response = await fetch(`https://api.spotify.com/v1/me/player/volume?volume_percent=${volume}&device_id=${encodeURIComponent(deviceId)}`, {
          method: 'PUT',
          headers: {
              'Authorization': `Bearer ${accessToken}`,
              'Content-Type': 'application/json'
...
app.post('/create-playlist', async (req, res) => {
  ...
  try {
    ...
    const playlistsResponse = await axios.get(`https://api.spotify.com/v1/users/${req.session.userId}/playlists`, {
      headers: { 'Authorization': `Bearer ${req.session.accessToken}` },
    });
    const findtunePlaylist = playlistsResponse.data.items.find(playlist => playlist.name === name);
    ...
    console.error('Failed to create or find playlist:', error.response?.data || error.message);
...
app.post('/add-track', async (req, res) => {
  ...
  if (!req.session.accessToken || !playlistId || !uri) {
    return res.status(403).json({ error: 'Missing required parameters or access token' });
  }
